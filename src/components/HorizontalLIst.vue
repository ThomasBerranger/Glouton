...
function getTimeLeftForHuman(product) {
  ...
  let humanMessage = `${moment(nearestExpirationTimestampDate)
    .startOf("day")
    .from(moment().startOf("day"))}`;

  if (
    moment(nearestExpirationTimestampDate).startOf("day").valueOf() ===
...
<template>
  <div class="mt-5 w-11/12 mx-auto list-height">
    ...
    <div
      v-else-if="props.products.values.length < 1"
    ...
    <RouterLink v-for="product in props.products.values" :to="'/product/' + product.id">
      <div class="grid grid-cols-12 bg-white my-3 shadow-md py-2 rounded-sm">
        <img
          v-if="product.image"
          class="relative h-16 col-span-3 place-self-center rounded-sm"
          :src="product.image"
          :alt="product.name"
        />
        <img
        ...
        <div class="col-span-9 my-auto px-1 text-left">
          ...
          <span
            :class="
              ...
              '-color ' +
              getColor(product) +
              '-background-low-opacity px-1.5 py-1 text-xs'
            "
            >{{ getTimeLeftForHuman(product) }}</span
          >
        </div>
      </div>
    </RouterLink>
