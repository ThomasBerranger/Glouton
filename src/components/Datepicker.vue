...
let firstDayOfSelectedMonth = ref<string>(moment(props.date, 'DD-MM-YYYY', 'DD-MM-YYYY').startOf('month').format('YYYY-MM-DD'));
...
// Calendar constants
const days: string[] = ['Lu', 'Ma', 'Me', 'Je', 'Ve', 'Sa', 'Di'];
...
let showMonths = ref<boolean>(false);
let showYears = ref<boolean>(false);
...
function changeYear(newYear: number): void {
  firstDayOfSelectedMonth.value = moment(firstDayOfSelectedMonth.value)
      .add(newYear - moment(firstDayOfSelectedMonth.value).year(), 'year')
      .format('YYYY-MM-DD');
...
function changeMonth(newMonthSameYear: number | null, next = true): void {
  if (newMonthSameYear) {
...
<template>
  <div>
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-10 h-screen animate-overlay"
         @click="$emit('updateDate', null)">

      <div class="w-10/12 mx-auto bg-white/100 z-20 rounded animate-modal" @click.stop>
        ...
        <Transition mode="out-in">
            ...
            <div v-for="(day, dayIdx) in calendar" :key="day.date"
              ...
              <button type="button" class="mx-auto flex h-8 w-8 items-center justify-center"
                  ...
                  !day.currentDay && !day.selected && !day.selectedMonth && 'text-gray-400',
                      ...
                      @click="$emit('updateDate', moment(day.date, 'L').format('YYYY-MM-DD'))">
                {{ day.dayNumber }}
              </button>
            </div>
          </div>
          ...
          <div v-else-if="showMonths" class="mt-2 grid grid-cols-3 gap-2 px-2 text-center text-sm">
            <div v-for="(month, monthNumber) in months" :key="month" class="my-2 py-2 rounded-lg ring-inset"
                 :class="[selectedMonth.number === parseInt(String(monthNumber)) ? 'ring-2 ring-indigo-600' : 'ring-1 ring-gray-300']"
